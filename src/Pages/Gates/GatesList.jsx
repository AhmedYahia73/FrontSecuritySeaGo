...
export default function GatesList() {
  const { data, loading, error } = useGet({ url: "/security/home/gates" });
  const navigate = useNavigate();

  if (loading) return <FullPageLoader />;
  if (error) return <div className="p-8 text-center text-red-500 font-semibold">Error loading gates. Please try again.</div>;

  let gates = [];
  if (Array.isArray(data)) gates = data;
  else if (data?.data && Array.isArray(data.data)) gates = data.data;
  else if (data?.gate && Array.isArray(data.gate)) gates = data.gate;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white border border-gray-100 shadow-[0_2px_20px_rgb(0,0,0,0.02)] p-10 md:p-14 lg:p-16 flex flex-col md:flex-row md:items-center justify-between gap-8 z-10" style={{ marginBottom: '80px' }}>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div>
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-2">Assigned Gates</h2>
              <p className="text-gray-500 text-sm md:text-base max-w-xl leading-relaxed">
                Monitor and manage the main external gates, working hours, and operational status.
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 shrink-0">
             <div className="flex flex-col items-center justify-center bg-gray-50 border border-gray-100 px-8 py-4 rounded-2xl">
                <span className="text-3xl font-black text-[#0AA6A5] leading-none mb-1">{gates.length}</span>
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Gates</span>
             </div>
          </div>
        </div>
        ...
        {gates.length === 0 ? (
          <div className="text-center text-gray-500 py-16 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm flex flex-col items-center justify-center">
            <DoorOpen className="w-16 h-16 text-gray-300 mb-4" />
            <p className="text-lg font-semibold">No gates assigned to you.</p>
          </div>
        ) : (
                ...
                <div className="h-48 w-full relative overflow-hidden bg-gradient-to-br from-teal-400 to-[#0AA6A5]">
                  {gate.image_link ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <DoorOpen className="w-20 h-20 text-white/40" />
                    </div>
                  )}
