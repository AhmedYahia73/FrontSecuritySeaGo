...
import { Clock, Waves, MapPin } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { useNavigate } from "react-router-dom";

export default function BeachesList() {
  const { data, loading, error } = useGet({ url: "/security/home/beaches" });
  const navigate = useNavigate();

  if (loading) return <FullPageLoader />;
  if (error) return <div className="p-8 text-center text-red-500 font-semibold">Error loading beaches. Please try again.</div>;

  let beaches = [];
  if (Array.isArray(data)) beaches = data;
  else if (data?.data && Array.isArray(data.data)) beaches = data.data;
  else if (data?.beach && Array.isArray(data.beach)) beaches = data.beach;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white border border-gray-100 shadow-[0_2px_20px_rgb(0,0,0,0.02)] p-10 md:p-14 lg:p-16 flex flex-col md:flex-row md:items-center justify-between gap-8 z-10" style={{ marginBottom: '80px' }}>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div>
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-2">Assigned Beaches</h2>
              <p className="text-gray-500 text-sm md:text-base max-w-xl leading-relaxed">
                Monitor and manage the active status and working hours of the beaches under your supervision.
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 shrink-0">
             <div className="flex flex-col items-center justify-center bg-gray-50 border border-gray-100 px-8 py-4 rounded-2xl">
                <span className="text-3xl font-black text-[#0AA6A5] leading-none mb-1">{beaches.length}</span>
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Beaches</span>
             </div>
          </div>
        </div>
        ...
        {beaches.length === 0 ? (
          <div className="text-center text-gray-500 py-16 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm flex flex-col items-center justify-center">
            ...
            <p className="text-lg font-semibold">No beaches assigned to you.</p>
          </div>
        ) : (
            ...
            {beaches.map((beach) => (
              ...
              >
                ...
                <div className="h-48 w-full relative overflow-hidden bg-gradient-to-br from-teal-400 to-[#0AA6A5]">
                  {beach.image_link ? (
                    <img 
                      src={beach.image_link.replace('storage//', 'storage/')} 
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Waves className="w-20 h-20 text-white/40" />
                    </div>
                  )}
                  ...
                  <div className="absolute top-4 right-4">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-md backdrop-blur-md ${beach.status === 1 ? 'bg-green-500/90 text-white' : 'bg-red-500/90 text-white'}`}>
                      {beach.status === 1 ? 'Active' : 'Inactive'}
                    </div>
                  </div>
                </div>
                
                {/* Card Content */}
                <CardContent className="p-10 pt-8 flex-1 flex flex-col relative z-10 bg-white">
                  <h3 className="text-2xl font-bold text-gray-900 group-hover:text-[#0AA6A5] transition-colors mb-4 line-clamp-1">
                    {beach.name || beach.translations?.[0]?.value}
                  </h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                    {(beach.from && beach.to) ? (
                      <div className="flex items-center gap-2 text-gray-600 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                        ...
                        <span className="text-sm font-semibold">{beach.from.slice(0,5)} - {beach.to.slice(0,5)}</span>
                      </div>
                    ) : (
