...
import { ShieldCheck, Users, ArrowLeft, LogIn, UserCheck } from "lucide-react";

export default function InsideGateDashboard() {
  const { id } = useParams();
  const navigate = useNavigate();
  
  // Notice: The backend may not have this endpoint yet. We will mock the UI for now, 
  // or use whatever data returns if the endpoint exists.
  const { data, loading, error } = useGet({ url: `/security/home/entrance_inside_gate?gate_id=${id}` });

  if (loading) return <FullPageLoader />;
  
  // If endpoint is 404, we can render a beautiful empty state or mock data for the modern experience.
  const stats = data || { total: 0 };
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center relative z-10" style={{ gap: '1.5rem' }}>
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-teal-400 to-[#0AA6A5] p-[2px] shadow-lg shadow-teal-500/30">
              <div className="w-full h-full bg-white rounded-2xl flex items-center justify-center">
                <ShieldCheck className="w-10 h-10 text-[#0AA6A5]" />
              </div>
            </div>
            <div>
              <h1 className="text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600" style={{ marginBottom: '0.5rem' }}>
                Inside Gate Dashboard
              </h1>
              <p className="text-gray-500 text-lg max-w-xl">
                Monitor internal entrance statistics and activity for this specific gate.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" style={{ gap: '1.5rem', marginBottom: '2.5rem' }}>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 transition-all duration-300" style={{ padding: '3rem' }}>
            ...
            <h3 className="text-5xl font-bold text-gray-900">{stats.user_inside_gate || stats.total || stats.entrance || 0}</h3>
          </div>
        </div>

        {error && (
          <div className="bg-red-50 text-red-500 rounded-2xl border border-red-100" style={{ marginTop: '2rem', padding: '1.5rem' }}>
            Note: The backend endpoint for Inside Gates statistics could not be loaded or may not exist yet.
          </div>
        )}
        ...
        <div className="relative overflow-hidden rounded-[2.5rem] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 flex flex-col md:flex-row items-center justify-between w-full" style={{ marginBottom: '4rem', padding: '3.5rem', gap: '2.5rem' }}>
          ...
          {/* Decorative Elements */}
          <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/3 w-64 h-64 bg-indigo-50 rounded-full blur-[60px] opacity-70 pointer-events-none"></div>
          <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/4 w-48 h-48 bg-indigo-500 rounded-full blur-[80px] opacity-5 pointer-events-none"></div>
          
          <div className="relative z-10 flex items-center" style={{ gap: '1.5rem' }}>
            <div className="w-20 h-20 shrink-0 rounded-3xl bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 flex items-center justify-center shadow-sm">
            ...
            <div>
              <h3 className="text-3xl font-bold text-gray-900 tracking-tight" style={{ marginBottom: '0.75rem' }}>
                Inside Gate Entrance Logging
              </h3>
              <p className="text-gray-500 text-lg max-w-lg leading-relaxed">
                Log the entrance of registered village owners and renters through this inside gate. Search by exact phone number to authorize their entry.
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 shrink-0 w-full md:w-auto">
            <button 
              onClick={() => navigate(`/inside-gates/${id}/users`)}
              className="group flex items-center justify-center w-full md:w-auto bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white text-lg font-bold rounded-2xl transition-all duration-300 shadow-[0_8px_20px_rgba(99,102,241,0.3)] hover:shadow-[0_12px_25px_rgba(99,102,241,0.5)] hover:-translate-y-1"
