...
export default function InsideGateVisitorsSearch() {
  const { id } = useParams();
  ...
  const fetchVisitors = async () => {
    setLoading(true);
    try {
      let url = `/security/beach_qr/visitors_insde_gate?inside_gate_id=${id}&page=${page}&locale=en`;
  ...
  const handleConfirmEntrance = async () => {
    ...
    try {
      const response = await api.post("/security/beach_qr/entrance_visitor_inside_gate_user", {
        inside_gate_id: id,
        visitor_id: selectedVisitor.id,
        appartment_id: selectedVisitor.appartment_id,
        locale: "en"
      });
