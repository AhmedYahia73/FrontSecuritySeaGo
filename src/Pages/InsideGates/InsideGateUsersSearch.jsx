...
import { Search, ArrowLeft, LogIn, MapPin, CheckCircle2, User, Phone, Calendar, Clock, BadgeCheck } from "lucide-react";
import { toast } from "react-toastify";

export default function InsideGateUsersSearch() {
  const { id } = useParams();
  ...
  const handleConfirmEntrance = async () => {
    ...
    try {
      const response = await api.post("/security/beach_qr/entrance_inside_gate_user", {
        inside_gate_id: id,
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 text-gray-800 font-sans selection:bg-indigo-500 selection:text-white pb-24 relative overflow-hidden">
      
      {/* Abstract Background Elements */}
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-indigo-500/5 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
      ...
      <div className="relative z-10 border-b border-gray-200 bg-white shadow-sm" style={{ padding: '2rem 1.5rem' }}>
        <div className="max-w-6xl mx-auto">
          <button 
            onClick={() => navigate(-1)} 
            className="group flex items-center gap-2 text-sm text-gray-500 hover:text-indigo-600 transition-all font-medium mb-6 w-fit"
          >
            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
            Back to Inside Gate Dashboard
          </button>
          
          <div className="flex flex-col gap-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 text-gray-900">
                Inside Gate Entrance Search
              </h1>
              <p className="text-gray-500 text-base max-w-xl">
                Search for owners and renters by phone number to log their entrance through this inside gate.
              </p>
            </div>
            ...
            <form onSubmit={handleSearch} className="relative w-full flex gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-gray-50 border border-gray-200 text-gray-900 text-lg placeholder:text-gray-400 focus:bg-white focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all outline-none shadow-sm hover:border-gray-300"
                />
              </div>
              <button 
                type="submit"
                disabled={loading || !searchQuery.trim()}
                className="h-[4rem] px-8 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-sm shadow-indigo-600/20 flex items-center justify-center gap-2 disabled:opacity-50 min-w-[140px]"
              >
                {loading ? (
        ...
        {loading ? (
          <div className="flex justify-center items-center py-20">
            <div className="relative w-16 h-16 flex items-center justify-center">
              <div className="absolute inset-0 border-4 border-gray-100 border-t-indigo-600 rounded-full animate-spin"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.map((user, index) => {
              ...
              return (
                <div key={user.id || index} className="group relative bg-white rounded-2xl border border-gray-200 flex flex-col shadow-sm hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-300 hover:border-indigo-500/40" style={{ padding: '1.5rem' }}>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1.5">
                          <p className="text-xs font-semibold text-gray-500 flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-full border border-gray-100">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span> Registered User
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 rounded-xl border border-gray-100 flex flex-col justify-center" style={{ padding: '1rem' }}>
                        <span className="text-gray-500 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest mb-1.5">
                          <MapPin size={12} className="text-indigo-600" /> Unit
                        </span>
                  ...
                  <button
                    ...
                    className="mt-auto w-full bg-white text-indigo-600 border-2 border-indigo-600 rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition-all duration-300 flex items-center justify-center gap-2 text-sm shadow-sm"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative bg-white border border-gray-100 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl scale-in-center">
            ...
            <div style={{ padding: '2rem' }}>
              <div className="w-16 h-16 bg-indigo-600/10 rounded-2xl flex items-center justify-center text-indigo-600 mx-auto" style={{ marginBottom: '1.5rem' }}>
              ...
              <p className="text-center text-gray-500 text-sm leading-relaxed" style={{ marginBottom: '2rem' }}>
                Confirm entry for <span className="text-indigo-600 font-semibold">{selectedUser?.name || 'User'}</span>.
              </p>
              
              <div style={{ marginBottom: '2rem' }}>
                <label className="block text-gray-700 text-sm font-bold" style={{ marginBottom: '0.5rem' }}>Select Destination Unit</label>
                <select 
                  className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 outline-none transition-all"
                  value={selectedUnit}
              ...
              <div className="flex items-center justify-center" style={{ gap: '0.75rem' }}>
                ...
                <button
                  ...
                  className="flex-1 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-all shadow-sm shadow-indigo-600/30 disabled:opacity-50 flex items-center justify-center gap-2 text-sm"
      ...
      {isResultModalOpen && responseData && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300" style={{ padding: '1rem' }}>
          <div className="relative bg-white border border-gray-100 rounded-[2rem] w-full max-w-lg overflow-hidden shadow-2xl scale-in-center">
            ...
            {/* Header / Banner */}
            <div className="flex flex-col items-center justify-center text-white relative overflow-hidden bg-gradient-to-br from-indigo-600 to-indigo-500" style={{ padding: '2rem' }}>
              ...
              <div className="flex items-center gap-2 relative z-10">
                <span className="bg-white/20 text-white text-xs font-bold uppercase tracking-wider rounded-full border border-white/30 shadow-sm backdrop-blur-sm" style={{ padding: '0.375rem 1rem' }}>
                  {responseData.user_type || "Owner"}
                </span>
                
                {responseData.is_visitor && (
                  <span className="bg-orange-500/80 text-white text-xs font-bold uppercase tracking-wider rounded-full border border-orange-400/50 shadow-sm backdrop-blur-sm" style={{ padding: '0.375rem 1rem' }}>
                    Visitor
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2" style={{ gap: '1rem', marginBottom: '1.5rem' }}>
                <div className="bg-gray-50 rounded-2xl border border-gray-100 flex flex-col" style={{ padding: '1rem' }}>
                  <span className="text-gray-400 flex items-center text-[10px] font-bold uppercase tracking-widest" style={{ gap: '0.375rem', marginBottom: '0.375rem' }}>
                    <MapPin size={12} className="text-indigo-600" /> Unit Name
                  </span>
                ...
                <div className="bg-gray-50 rounded-2xl border border-gray-100 flex flex-col" style={{ padding: '1rem' }}>
                  <span className="text-gray-400 flex items-center text-[10px] font-bold uppercase tracking-widest" style={{ gap: '0.375rem', marginBottom: '0.375rem' }}>
                    <BadgeCheck size={12} className="text-indigo-600" /> Status
                  </span>
                  <span className="font-semibold text-gray-900 text-lg capitalize truncate">
                    {responseData.is_visitor ? 'Visitor' : 'Resident'}
                  </span>
                </div>
              </div>

              {/* Time Details Info */}
              <div className="bg-indigo-50/50 rounded-2xl border border-indigo-100 p-4 mb-6 flex flex-col gap-3">
                 <span className="text-indigo-600/80 flex items-center justify-center text-[11px] font-bold uppercase tracking-widest gap-2">
                   <Clock size={14} /> Last Visit
                 </span>
                 <div className="grid grid-cols-2 gap-4 text-center mt-1">
                    <div className="flex flex-col">
                       <span className="text-indigo-400 text-[10px] font-bold uppercase tracking-widest mb-1">Date</span>
                       <span className="font-bold text-gray-900 text-base">{responseData.date || "N/A"}</span>
                    </div>
                    <div className="flex flex-col border-l border-indigo-200">
                       <span className="text-indigo-400 text-[10px] font-bold uppercase tracking-widest mb-1">Time</span>
                       <span className="font-bold text-gray-900 text-base">{responseData.time || "N/A"}</span>
                    </div>
                 </div>
              </div>
