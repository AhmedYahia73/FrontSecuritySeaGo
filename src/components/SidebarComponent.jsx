...
const navItems = [
  ...
  { label: "Profile", to: "/profile", icon: <User size={20} />, permissionKey: "Home" },
  {
    label: "Notification",
    to: "/notification",
    icon: <Bell size={20} />,
...
export function AppSidebar() {
  ...
  useEffect(() => {
    ...
    const userPermissionStr = localStorage.getItem("userPermission");
    ...
    if (userData) {
      try {
        ...
        if (userRoles.length > 0) {
          ...
          allowedItems = filterItems(itemsToFilter, rolesObject);
        } else if (userPermissionStr) {
          // Security Guard logic
          const perms = JSON.parse(userPermissionStr);
          allowedItems.push({ label: "Home", to: "/", icon: <Home size={20} /> });
          allowedItems.push({ label: "Profile", to: "/profile", icon: <User size={20} /> });
          
          if (perms.gate) allowedItems.push({ label: "Gates", to: "/gates", icon: <Map size={20} /> });
          if (perms.beach) allowedItems.push({ label: "Beaches", to: "/beaches", icon: <Map size={20} /> });
          if (perms.pool) allowedItems.push({ label: "Pools", to: "/pools", icon: <Map size={20} /> });
          if (perms.inside_gates) allowedItems.push({ label: "Inside Gates", to: "/inside-gates", icon: <Shield size={20} /> });
        }
